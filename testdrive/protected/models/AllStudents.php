<?php
class AllStudents extends CFormModel
{
	public $StudentID;
	public $LastName;
	public $FirstName;
	...
	{
		return array(
				array('Email, StudentID, LastName, FirstName, CourseName, CourseNumber, Semester, Year, Status','safe','on'=>'search')
		);
	}
	...
	public function getSqlDataProvider($Semester, $Year)
	{
		...
		{
			$Condition .= " AND Students.Username REGEXP '" . $this->Username . "'";
		}
		if (!empty($this->Email))
		{
			$Condition .= " AND Students.Email REGEXP '" . $this->Email . "'";
		}
				 
		
		$query = "SELECT	Students.StudentID, Students.LastName, Students.FirstName, Students.Username,
				 Students.Email, course.Name as CourseName, course.Number as CourseNumber, 
				 class.Section, class.Semester, class.Year, enrolled.Status, enrolled.StatusChange
			FROM 	Students
